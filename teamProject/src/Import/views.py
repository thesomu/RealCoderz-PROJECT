...
def portalLogin_view(request):
    ...
    if request.method == "POST":
        ...
        if((userId == 'harsh1234') and (password == "1998")):
            ...
            login.warning("Authentication failed. Wrong login details. The login page is reloaded")
...
def insertImportOrder_view(request):
    ...
    if request.method == "POST":
        ...
        order.save()
        logger.info("Record inserted in database successfully and request redirected to display orders page.")
...
def displayImportOrders_view(request):
    ...
    if 'user' in request.session:
        ...
        logger.info("Record fetched successfully and request redirected to importOrders page.")
...
def editImportOrders_view(request,id):
    ...
    logger.info("Record fetched on the basis of id: "+str(id)+" and request redirected to editImportPage")
...
def updateImportOrders_view(request, id):
    ...
    order.save()
    logger.info("Record with id: "+str(id)+" updated successfully and request redirected to the displayOrders page.")
    return redirect('/displayOrders')
